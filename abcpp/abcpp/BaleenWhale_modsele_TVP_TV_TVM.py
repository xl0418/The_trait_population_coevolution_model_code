...
sampleparam_TV = DVParamLiang(gamma=1, a=1, K=K_TV,h=1, nu=nu, r=1, theta=meantrait,V00=.1,V01=.1, Vmax=100, inittrait=meantrait, initpop=1e5,
                                initpop_sigma=10.0, break_on_mu=False)
sampleparam_TVM = DVParamLiang(gamma=1, a=1, K=K_TVM,h=1, nu=nu, r=1, theta=meantrait,V00=.1,V01=.1, Vmax=100, inittrait=meantrait, initpop=1e5,
                                initpop_sigma=10.0, break_on_mu=False)
...
for g in range(generations):
    ...
    q5_TVP = np.argsort(fitness[g, :population-1])[-int(population // 20)]  # best 25%
    q5_TV = np.argsort(fitness[g, population:2*population-1])[-int(population // 20)]+population  # best 25%
    q5_TVM = np.argsort(fitness[g, 2*population:3*population-1])[-int(population // 20)]+2*population  # best 25%
