...
params_TVM[:, 6] = np.random.uniform(prior_TVM[8], prior_TVM[9], params_TVM.shape[0])  # randomize 'theta'
...
for g in range(generations):
    ...
    if TVP_sample_length>0:
        ...
        if num_valid_sims_TVP > 0:
            ...
            pic_ordered_list = num_cores.starmap(pic_compute, zip(repeat(tree_sim), Z_modelTVP, repeat(taxa1),range(num_valid_sims_TVP)))
    ...
    if TV_sample_length>0:
        ...
        pic_ordered_list = num_cores.starmap(pic_compute, zip(repeat(tree_sim), Z_modelTV, repeat(taxa1),
    ...
    if TVM_sample_length>0:
        ...
        pic_ordered_list = num_cores.starmap(pic_compute, zip(repeat(tree_sim), Z_modelTVM, repeat(taxa1),
    ...
    chosengamma_TVP,chosena_TVP,chosennu_TVP,chosenvm_TVP,chosentheta_TVP = np.mean(params_TVP[previous_bestfitted_index_TVP,0]),\
                                            ...
                                            np.mean(params_TVP[previous_bestfitted_index_TVP, 6])

    chosengamma_TV,chosena_TV,chosennu_TV,chosenvm_TV,chosentheta_TV = np.mean(params_TV[previous_bestfitted_index_TV,0]),\
                                            np.mean(params_TV[previous_bestfitted_index_TV,1]),\
                                                ...
                                                np.mean(params_TV[previous_bestfitted_index_TV, 9]), \
                                                        np.mean(params_TV[previous_bestfitted_index_TV, 6])

    chosengamma_TVM,chosena_TVM,chosennu_TVM,chosenvm_TVM,chosentheta_TVM = np.mean(params_TVM[previous_bestfitted_index_TVM,0]),\
                                            np.mean(params_TVM[previous_bestfitted_index_TVM,1]),\
                                                ...
                                                np.mean(params_TVM[previous_bestfitted_index_TVM, 9]), \
                                                        np.mean(params_TVM[previous_bestfitted_index_TVM, 6])

    print('Mean estimates: TVP gamma: %.3e ; a: %.3e ; nu: %.3e ; Vm : %f; theta : %f'  % ( chosengamma_TVP,chosena_TVP,chosennu_TVP,chosenvm_TVP,chosentheta_TVP))
    print('Mean estimates: TV gamma: %.3e ; a: %.3e ; nu: %.3e ; Vm : %f; theta : %f'  % ( chosengamma_TV,chosena_TV,chosennu_TV,chosenvm_TV,chosentheta_TV))
    print('Mean estimates: TVM gamma: %.3e ; a: %.3e ; nu: %.3e ; Vm : %f; theta : %f'  % ( chosengamma_TVM,chosena_TVM,chosennu_TVM,chosenvm_TVM,chosentheta_TVM))
...
para_data = {'model_data': model_data,'fitness': fitness,'Z': Z,
             ...
             'gamma_data_TVM': gamma_data_TVM,'a_data_TVM': a_data_TVM, 'nu_data_TVM': nu_data_TVM,
